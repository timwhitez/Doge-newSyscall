...
import (
	...
	"errors"
	"fmt"
	"github.com/timwhitez/Doge-newSyscall/pkg/findegg"
	"math/rand"
	"reflect"
	"syscall"
	"time"
...
func init() {
	ptr := findegg.FindEgg([]byte{0x65, 0x67, 0x67, 0x63, 0x61, 0x6c, 0x6c}, reflect.ValueOf(pS).Pointer())
	p = ptr + 7
	d = ptr + 25
}
...
func PrepareSyscall(sysid uint16) uintptr {
	callAddr := GetCall("", hookedapi, str2sha1)
	syscall.Syscall(p, 2, uintptr(sysid), callAddr, 0)
	return d
}
